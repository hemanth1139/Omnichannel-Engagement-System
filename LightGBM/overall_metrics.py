...
# 1. Save CSV in exact 5-row template format
csv_path = os.path.join(OUTPUT_DIR, "overall_model_metrics.csv")
...
with open(report_path, "w") as f:
    ...
    f.write(
        "OMNICHANNEL HCP LIGHTGBM MODEL EVALUATION METRICS SUMMARY (TEMPLATE FORMAT)\n"
    )
    f.write(
        "================================================================================\n\n"
    )
    f.write(df_template.to_string(index=False) + "\n\n")

print(
    f"Successfully generated dynamic metrics in exact template format:\n  - {csv_path}\n  - {report_path}\n"
)
print(df_template.to_string(index=False))
